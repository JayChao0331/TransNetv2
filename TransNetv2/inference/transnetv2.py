...
class TransNetV2:
    ...
    def predict_video(self, video_fn: str):
        ...
        video_stream, err = ffmpeg.input(video_fn).filter('fps', fps=5, round='up').output(
...
def main():
    ...
    for file in args.files:
        ...
        # define save name
        file_lst = file.split('/')
        save_name = '/'.join(file_lst)

        # predictions = np.stack([single_frame_predictions, all_frame_predictions], 1)
        # np.savetxt(save_name + ".predictions.txt", predictions, fmt="%.6f")

        scenes = model.predictions_to_scenes(single_frame_predictions)
        np.savetxt(save_name + ".scenes.txt", scenes, fmt="%d")

        if args.visualize:
            if os.path.exists(save_name + ".vis.png"):
                print(f"[TransNetV2] {save_name}.vis.png already exists. "
                      f"Skipping visualization of video {save_name}.", file=sys.stderr)
                continue

            pil_image = model.visualize_predictions(
                video_frames, predictions=(single_frame_predictions, all_frame_predictions))
            pil_image.save(save_name + ".vis.png")
